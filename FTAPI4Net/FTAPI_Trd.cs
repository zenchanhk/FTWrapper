...
namespace Futu.OpenApi
{
    ...
    {
        void OnReply_GetAccList(FTAPI_Conn client, int nSerialNo, TrdGetAccList.Response rsp); //获取交易账户列表回调
        void OnReply_UnlockTrade(FTAPI_Conn client, int nSerialNo, TrdUnlockTrade.Response rsp); //解锁回调
        void OnReply_SubAccPush(FTAPI_Conn client, int nSerialNo, TrdSubAccPush.Response rsp); //订阅接收推送数据的交易账户回调
        void OnReply_GetFunds(FTAPI_Conn client, int nSerialNo, TrdGetFunds.Response rsp); //获取账户资金回调
        void OnReply_GetPositionList(FTAPI_Conn client, int nSerialNo, TrdGetPositionList.Response rsp); //获取账户持仓回调
        void OnReply_GetMaxTrdQtys(FTAPI_Conn client, int nSerialNo, TrdGetMaxTrdQtys.Response rsp); //获取最大交易数量回调
        void OnReply_GetOrderList(FTAPI_Conn client, int nSerialNo, TrdGetOrderList.Response rsp); //获取当日订单列表回调
        void OnReply_PlaceOrder(FTAPI_Conn client, int nSerialNo, TrdPlaceOrder.Response rsp); //下单回调
        void OnReply_ModifyOrder(FTAPI_Conn client, int nSerialNo, TrdModifyOrder.Response rsp); //修改订单回调
        void OnReply_GetOrderFillList(FTAPI_Conn client, int nSerialNo, TrdGetOrderFillList.Response rsp); //获取当日成交列表回调
        void OnReply_GetHistoryOrderList(FTAPI_Conn client, int nSerialNo, TrdGetHistoryOrderList.Response rsp); //获取历史订单列表回调
        void OnReply_GetHistoryOrderFillList(FTAPI_Conn client, int nSerialNo, TrdGetHistoryOrderFillList.Response rsp); //获取历史成交列表回调
        void OnReply_UpdateOrder(FTAPI_Conn client, int nSerialNo, TrdUpdateOrder.Response rsp); //订单状态变动通知(推送)回调
        void OnReply_UpdateOrderFill(FTAPI_Conn client, int nSerialNo, TrdUpdateOrderFill.Response rsp); //成交通知(推送)回调
    }
                    ...
                    case ProtoID.TrdUpdateOrder://订单状态变动通知(推送)
                        {
                            ...
                            this.trdCallback.OnReply_UpdateOrder(this, serialNo, rsp);
                        }
                        break;
                    case ProtoID.TrdUpdateOrderFill://成交通知(推送)
                        {
                            ...
                            this.trdCallback.OnReply_UpdateOrderFill(this, serialNo, rsp);
                        }
                        break;
                }
            }
